...
public class InstitutionAttributes {
  ...
  * @throws IOException if the JSON Element is invalid with respect to InstitutionAttributes
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!InstitutionAttributes.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in InstitutionAttributes is not found in the empty JSON string", InstitutionAttributes.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!InstitutionAttributes.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `InstitutionAttributes` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : InstitutionAttributes.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!InstitutionAttributes.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'InstitutionAttributes' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<InstitutionAttributes> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(InstitutionAttributes.class));

       return (TypeAdapter<T>) new TypeAdapter<InstitutionAttributes>() {
           @Override
           public void write(JsonWriter out, InstitutionAttributes value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public InstitutionAttributes read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of InstitutionAttributes given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of InstitutionAttributes
  * @throws IOException if the JSON string is invalid with respect to InstitutionAttributes
  */
  public static InstitutionAttributes fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, InstitutionAttributes.class);
  }

 /**
  * Convert an instance of InstitutionAttributes to an JSON string
  *
  * @return JSON string
  */
